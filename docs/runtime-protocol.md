...
Builds the configured host app and returns structured diagnostics:
...
When the build fails, `success` is `false` and `diagnostics` contains up to 20 `error:` lines from the Xcode build output. Used by the extension after AI adapter generation to verify that generated code compiles.
...
The "Generate Stubs" command creates preview adapter stubs — one function per discovered SwiftUI view — in a dedicated `SwiftUIExplorerPreviewAdapters.swift` file next to the app entry file. The `@main` entry file gets stable bootstrap wiring (registry, launch selection, manifest generation) inside a `// swiftui-explorer:begin` / `end` block. Rerunning setup regenerates the bootstrap block but preserves the adapter file — existing adapter implementations are never overwritten, and only new views get stubs appended.

### AI-Assisted Generation

The "Generate With AI" command uses the editor's built-in language model to fill in adapter bodies automatically. For each placeholder adapter, the extension:

1. Reads the view's source file and related files in the same directory.
2. Sends a focused prompt asking the model to produce a working initializer.
3. Replaces the placeholder body with the model's output.
4. Calls `POST /api/v1/validate` to verify the generated code compiles.
5. Updates the manifest `status` from `"placeholder"` to `"configured"` for adapters that no longer contain `ContentUnavailableView`.

AI-generated code is written into `SwiftUIExplorerPreviewAdapters.swift` as normal Swift. It is app-owned, editable, and does not depend on any AI service at runtime.

## Design Constraints

- The extension should never shell out to `xcodebuild` directly.
- The runtime should own all simulator process control and build validation.
...
- The system does not try to auto-instantiate arbitrary views at runtime. Real rendering comes from explicit, app-owned adapter functions — written by the developer or generated by AI.
